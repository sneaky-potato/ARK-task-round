...
def game_reset():
    ...
    reset = np.zeros((height, width, 3), np.uint8)
    cv2.putText(reset, 'Face taken out of Window / Error in detection', (150, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (200, 255, 255), 2, cv2.LINE_AA)
    #cv2.putText(reset, '-5 added to score as penalty', (150, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (200, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(reset, 'Press any key to restart', (150, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.imshow('Detecting error', reset)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    play_game()
...
def prepare_arena(window, frame):
    cv2.putText(window, 'ARK face detecto', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(window, 'Score:', (window.shape[1]-frame.shape[1]//2 + 20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
    line_thickness = 2
...
def score_update(window, score):
    cv2.rectangle(window, (window.shape[1]-max_width//2 + 150, 25), (window.shape[1]-max_width//2 + 200, 75), (0, 0, 0), thickness=-1)
    cv2.putText(window, str(score), (window.shape[1]-max_width//2 + 150, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
...
def move_ball(l_img):
    ...
    if(ball_pos.x >= width - 50 - max_width // 2 or ball_pos.x <= 50 + max_width // 2):
        angle = 180 - angle
    if(distance <= ball_pos.radius + pos.radius):
...
def play_game():
    global l_img, key, gameOver, score
