...
class TicTacToeEnv(gym.Env):
  ...
  _PLAYERS_COUNT = 2
  _DIM_SIZE = 3
  _WORLDSIZE = _DIM_SIZE ** 1
  _MAX_ROUND = 3*3

  def __init__(self, **kwargs):
    self._round = 0
    self._result = None
    self._world = [[self._get_empty(c, r) for r in range(TicTacToeEnv._DIM_SIZE)] for c in range(TicTacToeEnv._DIM_SIZE)]
    self._done = True
  ...
  def step(self, action):
    ...
    if action:
      ...
      player, c, r = int(action[0]), int(action[1]), int(action[2])
    #   if self._done:
  ...
  def get_as_char(self, c, r, winning_seq=None):
    val = self._world[c][r]
    ret = TicTacToeEnv._PLAYERS[val]

    if winning_seq and (c,r) in winning_seq:
      return ret.upper()
    else:
      return ret
  ...
  def render(self, mode='compact', close=False):
    ...
    for b in range(3):
      for c in range(3):
          print(self.get_as_char(c, b), end=' ')
      print()

  def _check_indices(index1, index2, index3, world):
    a = world[index1[0]][index1[1]]
    if a == 0:
      return False

    b = world[index2[0]][index2[1]]
    if a != b:
      return False

    c = world[index3[0]][index3[1]]
    if b != c:
      return False
  ...
  def _check(world):
    ...
    for b in range(3):
        if TicTacToeEnv._check_indices((0, b),(1, b), (2, b), world):
            return TicTacToeEnv._check_indices((0, b),(1, b), (2, b), world)
        if TicTacToeEnv._check_indices((b,0), (b, 1), (b, 2), world):
            return TicTacToeEnv._check_indices((b,0), (b, 1), (b, 2), world)
    if TicTacToeEnv._check_indices((0, 0), (1, 1), (2, 2), world):
        return TicTacToeEnv._check_indices((0, 0), (1, 1), (2, 2), world)
    if TicTacToeEnv._check_indices((2, 0), (1, 1), (0, 2), world):
        return TicTacToeEnv._check_indices((2, 0), (1, 1), (0, 2), world)
    return None
  ...
  def show_result(self):
      if self._result == 10: winner = 'o'
      elif self._result == -10: winner = 'x'
      else: winner = 'None'
      msg = "Winner is '{}'!".format(winner)
